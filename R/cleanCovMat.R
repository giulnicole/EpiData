...
#' @param p offset parameter to compute regularization of Beta matrix and M matrix
#'
#' @name cleanCovMat
#'
#' @return
...
GRconversion<- function(cleaned.list){
...
cleanCovMat <- function(X, Y, Z, max_na_cpg=0.5, max_na_ind=0.2, cpg_removal_threshold=10) {
  ...
  #objGR<- GRconversion(cleaned.list = results$Cleaned1)
  return(results)
